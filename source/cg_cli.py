            ...
            if line[0] == 'resetCanvas':
                ...
                for item_type, p_list, algorithm, color in item_dict.values():
                    if item_type == 'line':
                        ...
                        pass
                for transform_type, p_list, algorithm in transform_dict.values():
                ...
                k = 2
                dot_list = []
                while (line[k] != 'DDA' and line[k] != 'Bresenham'):
                    dot_list.append([int(line[k]), int(line(k + 1))])
                algorithm = line[k]
                item_dict[item_id] = ['polygon', dot_list, algorithm, np.array(pen_color)]
                ...
                dx = int(line[2])
                dy = int(line[3])
                transform_dict.append(['translate',[dx,dy],''])
            ...
